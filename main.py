...
def createModel(dim):
    ...
    l2 = Dense(150, kernel_initializer='uniform', kernel_regularizer=regularizers.l2(0.001), activation='relu')(d1)  #
    d2 = Dropout(0.2)(l2)
    # b2 =  BatchNormalization()(d2)

    l3 = Dense(200, kernel_initializer='uniform', kernel_regularizer=regularizers.l2(0.001), activation='relu')(d2)  #
    d3 = Dropout(0.2)(l3)
    # b3 = BatchNo rmalization()(l3)

    out = Dense(1, kernel_initializer='glorot_uniform', activation='sigmoid')(d3)
...
if __name__ == "__main__":
    trainFile = 'bnn_tcg_trainFile_200602_172216_BCA.root'
    examFile = 'bnn_tcg_examFile_200602_172442_CXJ.root'
    dim = 15

    model_name = 'exampleFCNC_tcg'
    #  load data
    ...
    # train
    history = model.fit(features_train, labels_train, epochs=200, batch_size=len(labels_train), shuffle=False, validation_data=(features_test, labels_test, weights_test), sample_weight=weights_train, )
    #  save model
